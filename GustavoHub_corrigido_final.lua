--[[ 
    ...
    - Aim Key: Right Click (configurável: teclado e Mouse 1-5)
...
local function SetAimBind(Input)
    if IsMouseButton(Input.UserInputType) then
...
local function SetAimToggleBind(Input)
    if IsMouseButton(Input.UserInputType) then
        AimToggleKey = Input.UserInputType
        AimToggleKeyButton.Text =
            "AIM-LOCK TOGGLE: " ..
            GetBindName(Input)
        return true
...
local function SetESPToggleBind(Input)
    if IsMouseButton(Input.UserInputType) then
        ESPToggleKey = Input.UserInputType
        ESPToggleKeyButton.Text =
            "ESP TOGGLE: " ..
            GetBindName(Input)
        return true
...
local function IsBoundInput(Input, Bind)
    if Bind == nil then
        return false
    end

    if IsMouseButton(Bind) then
...
UserInputService.InputBegan:Connect(
    function(Input, GameProcessed)
        ...
        --
        -- IMPORTANT:
        -- We intentionally handle binding BEFORE
        -- GameProcessed so mouse buttons consumed by
        -- the GUI can still be assigned.
        --------------------------------------------------

        if ChangingAimKey then
            if SetAimBind(Input) then
                ChangingAimKey = false
            end
            return
        end
        ...
        if ChangingAimToggleKey then
            if SetAimToggleBind(Input) then
                ChangingAimToggleKey = false
            end
            return
        end
        ...
        if ChangingESPToggleKey then
            if SetESPToggleBind(Input) then
                ChangingESPToggleKey = false
            end
            return
        end
        ...
        if IsBoundInput(Input, AimToggleKey) then
        ...
        if IsBoundInput(Input, ESPToggleKey) then
        ...
        if IsBoundInput(Input, AimKey) then
            ...
            -- Só escolhe um alvo quando começa
