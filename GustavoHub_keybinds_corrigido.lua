...
local function IsMouseBind(InputType)
...
local function InputName(Input)
    if IsMouseBind(Input.UserInputType) then
        return MouseBindNames[Input.UserInputType]
    end
...
local function IsSameBind(Input, Bind)
    if Bind == nil then
        return false
    end
...
local function CaptureBind(Input, Which)
    ...
    if Which == "Aim" then
        AimKey = IsMouseBind(Input.UserInputType)
    ...
    elseif Which == "AimToggle" then
        AimToggleKey = IsMouseBind(Input.UserInputType)
            and Input.UserInputType
            or Input.KeyCode
    ...
    elseif Which == "ESPToggle" then
        ESPToggleKey = IsMouseBind(Input.UserInputType)
            and Input.UserInputType
            or Input.KeyCode
...
UserInputService.InputBegan:Connect(
    function(Input, GameProcessed)
        ...
        -- CAPTURE MODE
        --
        -- This is deliberately BEFORE GameProcessed.
        -- Roblox GUI clicks often arrive with
        -- GameProcessed = true. Without this ordering,
        -- Left Click and mouse side buttons can be
        -- swallowed by the GUI and never become binds.
        --------------------------------------------------
        ...
        if IsSameBind(Input, AimToggleKey) then
        ...
        if IsSameBind(Input, ESPToggleKey) then
        ...
        if IsSameBind(Input, AimKey) then
