--[[ 
    ...
    - Aim Key: Right Click (teclado, Left/Right/Middle Click, Mouse 4/5)
...
AimToggleKeyButton.MouseButton1Click:Connect(
    function()
        ...
        AimToggleKeyButton.Text =
            "AIM-LOCK TOGGLE: PRESS KEY / MOUSE..."
    end
)
...
ESPToggleKeyButton.MouseButton1Click:Connect(
    function()
        ...
        ESPToggleKeyButton.Text =
            "ESP TOGGLE: PRESS KEY / MOUSE..."
    end
)
...
-- Use the enum NAME instead of directly indexing MouseButton4/5.
-- Roblox does not guarantee those enum members, and directly
-- referencing them can terminate the script while loading.

local MouseBindNames = {
...
-- Mouse 4/5 are accepted by this system when the current
-- Roblox client/executor exposes them as UserInputType names.
-- We deliberately use the names instead of Enum.UserInputType.MouseButton4/5,
-- because those enum constants are not present in standard Roblox.
...
local function IsMouseInput(Input)
    local n = GetInputTypeName(Input)
    return n == "MouseButton1"
        or n == "MouseButton2"
        or n == "MouseButton3"
        or n == "MouseButton4"
        or n == "MouseButton5"
...
local function GetInputName(Input)
    ...
    if n and MouseBindNames[n] then
        return MouseBindNames[n]
    end
...
local function GetBindValue(Input)
...
local function BindMatches(Input, Bind)
    ...
    if type(Bind) == "string" then
        return GetInputTypeName(Input) == Bind
    end

    return Input.KeyCode == Bind
end

local function CaptureBind(Input, Which)
    local value = GetBindValue(Input)
    local name = GetInputName(Input)
    ...
    if Which == "Aim" then
        AimKey = value
        AimKeyButton.Text = "AIM KEY: " .. name

    elseif Which == "AimToggle" then
        AimToggleKey = value
        AimToggleKeyButton.Text =
            "AIM-LOCK TOGGLE: " .. name

    elseif Which == "ESPToggle" then
        ESPToggleKey = value
        ESPToggleKeyButton.Text =
            "ESP TOGGLE: " .. name
    end
...
UserInputService.InputBegan:Connect(
    function(Input, GameProcessed)
        ...
        -- Capture a new bind BEFORE GameProcessed.
        -- GUI clicks otherwise get consumed by the interface.

        if ChangingAimKey then
            if CaptureBind(Input, "Aim") then
                ChangingAimKey = false
            end
            return
        end
        ...
        if ChangingAimToggleKey then
            ...
            if CaptureBind(Input, "AimToggle") then
                ChangingAimToggleKey = false
            end
            return
        end
        ...
        if ChangingESPToggleKey then
            ...
            if CaptureBind(Input, "ESPToggle") then
                ChangingESPToggleKey = false
            end
            return
        end
        ...
        if ChangingMinimizeKey then
            if Input.KeyCode ~= Enum.KeyCode.Unknown then
                ...
                MinimizeKeyButton.Text =
                    "MINIMIZE KEY: " .. Input.KeyCode.Name
                ChangingMinimizeKey = false
            end
            return
        end
        ...
        -- AIM-LOCK TOGGLE
        if BindMatches(Input, AimToggleKey) then
        ...
        -- ESP TOGGLE
        if BindMatches(Input, ESPToggleKey) then
        ...
        -- AIM HOLD
        if BindMatches(Input, AimKey) then
